...
export const COMPONENT_CATALOG: CatalogEntry[] = [
  ...
  },
  {
    name: "dashboard",
    layer: "archetype",
    purpose: "현황을 한눈에 훑는 첫 화면",
    slots: ["header", "stats", "panels(선택)", "table(선택)"],
    whenToUse: "로그인 직후 보는 화면. 여러 지표를 동시에 확인하고 이상을 발견하는 것이 목적일 때.",
    whenNotToUse:
      "특정 데이터를 찾아 들어가는 것이 목적이면 list 다. 지표가 하나뿐이면 만들지 않는다.",
  },
  {
    name: "settings",
    layer: "archetype",
    purpose: "켜고 끄는 항목들의 모음",
    slots: ["header", "groups[].options"],
    whenToUse: "값을 제출하는 것이 아니라 즉시 반영되는 선택들일 때.",
    whenNotToUse: "입력하고 저장 버튼을 누르는 흐름이면 form 이다.",
  },
  {
    name: "wizard",
    layer: "archetype",
    purpose: "여러 단계로 나눈 입력",
    slots: ["header", "steps[].fields", "submit"],
    whenToUse: "입력 항목이 많아 한 화면에 담으면 부담스러울 때. 단계마다 의미가 분명할 때.",
    whenNotToUse: "필드가 6개 이하이면 form 이 낫다. 단계를 나누면 이탈이 늘어난다.",
  },
  {
    name: "handoff",
    layer: "archetype",
    purpose: "자동 생성하지 않고 사람에게 넘기는 화면",
    slots: ["header", "reason", "suggestion(선택)"],
    whenToUse:
      "칸반 보드, 지도, 차트 중심 화면처럼 기존 원형으로 표현할 수 없을 때. 억지로 맞추지 않는다.",
    whenNotToUse: "조금 어색해도 기존 원형으로 표현 가능하다면 그쪽을 쓴다.",
  },
  {
    name: "AppShell",
    layer: "block",
    purpose: "IA 트리에서 만든 사이드바와 현재 위치",
    slots: ["tree", "activeId", "content"],
    whenToUse: "화면이 여러 개인 프로토타입. IA 트리만 있으면 자동으로 만들어진다.",
    whenNotToUse: "화면 하나만 보여줄 때는 필요 없다.",
  },
  {
    name: "PageHeader",
    layer: "block",
  ...
  },
  {
    ...
    purpose: "켜고 끄기",
    slots: ["checked"],
    whenToUse: "즉시 반영되는 설정. 저장 버튼이 없을 때.",
    whenNotToUse: "제출해야 반영되는 값이면 Checkbox 가 맞다.",
  },
  {
    name: "Tabs",
    layer: "primitive",
    purpose: "같은 맥락의 내용을 나눠 보기",
    slots: ["tabs", "panels"],
    whenToUse: "내용이 서로 배타적이고 동시에 볼 필요가 없을 때.",
    whenNotToUse: "비교하며 봐야 하는 내용. 탭 안의 내용은 검색에도 잡히지 않는다.",
  },
  {
    name: "Alert",
    layer: "primitive",
    purpose: "맥락에 붙는 안내 문구",
    slots: ["text"],
    whenToUse: "화면을 막지 않으면서 알려야 할 때.",
    whenNotToUse: "확인을 받아야 하는 내용이면 Dialog 다.",
  },
  {
    name: "Badge",
    layer: "primitive",
